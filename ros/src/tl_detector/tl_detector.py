...
###########################################################################################################################
    def __init__(self):
        ...
        ## subscriber to on/off images collection, to start collecting images
        ## rostopic pub -1 /tld/collect_images std_msgs/Bool True
        self.collect_images_dir = rospy.get_param('~images_directory', 'images')
        self.collect_images = False
        self.collect_images_sub = rospy.Subscriber('/tld/collect_images', Bool, self.collect_images_cb)
...
###########################################################################################################################
    def get_light_state(self, light_pos_x, light_pos_y, light_pos_z):
        ...
        if self.collect_images:
            if not os.path.exists(self.collect_images_dir): os.mkdir(self.collect_images_dir)
            now = rospy.Time.now()
            cv2.imwrite('{}/{}_{:0>9}.png'.format(self.collect_images_dir, now.secs, now.nsecs), cv_image)
